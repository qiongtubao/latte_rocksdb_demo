...
int main() {
    ...
    ropts = initReadOpts();
    latte_rocksdb_cf* db = openDbByColumnFamily(db_opts, "data.rocks/0");
    ...
    for (long long i = 0; i < count;i++) {
        ...
        writeDbCf(db, wopts, key, key_len,  value, VALUE_SIZE);
        flushDB(db->db, fopts);
    }
    ...
    // printf("rocksdb_compact_range once %lldus\n", (end_time - start_time) / count);
    sleep(21);
    free(value);
    rocksdb_options_destroy(db_opts);
    char* err = NULL;
    rocksdb_column_family_handle_destroy(db->handles[0]);
    // rocksdb_list_column_families_destroy( ,1);
    // if (err != NULL) {
    //     printf("close column family fail\n");
    // }
    rocksdb_close(db->db);
    return 1;
}